...
- Distance to buddy
Defensive:
- Number of food left on my side of the board
"""
# baselineTeam.py
# ---------------
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    return features
  def getWeights(self, gameState, action):
    return {'successorScore': 100, 'distanceToFood': -10, 'distanceToBuddy': 9, 'enemyDistance': -200, 'mydistsToCapsules': -100, 'foodLeft': -100, 'numCapsulesLeft': -500}
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    #stolen from last contest's baselinebot
    enemies = [successor.getAgentState(i) for i in self.getOpponents(successor)]
    invaders = [a for a in enemies if a.isPacman and a.getPosition() != None]
    features['numInvaders'] = len(invaders)
    if len(invaders) > 0:
    ...
    features['myFoodLeft'] = myFoodLeft
    return features
  def getWeights(self, gameState, action):
    return {'numInvaders': -100, 'onDefense': 100, 'invaderDistance': -10, 'stop': -100, 'reverse': -2, 'myFoodLeft': 10}
