"""
Students' Names: Christina Baek, Ellin Zhao
Contest Number: 2
Description of Bot:
We divided up the bots to take on two roles: the ghost (defensive agent) and the pacman
(offensive agent). They are both reflex agents. We tried implementing a minimax
in the chooseAction method that included all four agents, but the minimax sometimes
had computations that lasted longer than 1 second even with a depth of 1. The defensive
agent has the simple task of chasing after any enemy Pacman that goes into its area and
catching it, unless the ghost is scared. If there is no enemy Pacman, it gets as close to
the nearest enemy on the other enemy side of the board without actually entering it.
The offensive agent plays the role of eating the enemy food. Due to the fact that the
offensive agent is reflexive, we had to make sure that the agent stays very little time
on the enemy side of the board. As a result, we have the agent return to the team side if
it is carrying any pellets at all, instead of focusing on eating more pellets.
"""
