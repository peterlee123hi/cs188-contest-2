...
class OffensiveAgent(CaptureAgent):
  ...
  def chooseAction(self,gameState):
    ...
    # if self.foodNum >= 3 and (gameState.getAgentPosition(self.index) not in self.pathToExit):
    #   # calculate fastest pathToExit using BFS.
    #   visited = []
    #   fringe = util.Queue()
    #   fringe.push( (gameState, []) )
    ...
    #       # print "node.getAgentPosition(self.index)", node.getAgentPosition(self.index)
    ...
    #             # print "child", child
    #             # print "new directions", directions + [child.getAgentPosition(self.index)]
    #             fringe.push((child, directions + [child.getAgentPosition(self.index)]))
...
class DefensiveAgent(CaptureAgent):
  ...
  def getWeights(self,gameState, action):
    ...
    'normalGhosts': -30, 'eatGhost': 1.0, 'scaredGhosts': 0.1, 'stuck': -10, 'eatFood': 1, 'pathOnExitRoute': 10, 'closeToExitPos': -1, 'repeatMovement': -1}
