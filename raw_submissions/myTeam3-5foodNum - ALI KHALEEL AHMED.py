...
class OffensiveAgent(CaptureAgent):
  ...
  def chooseAction(self,gameState):
    ...
    # print self.getFeatures(gameState,nextAction)
    # print nextAction
    # if "West" in actions:
    #   print self.getFeatures(gameState, "West")
    #   print "West"
    # print "\n"
  ...
  def getFeatures(self, gameState, action):
    ...
    if self.foodNum >= 5: #and (newx, newy) in self.pathToExit:
  ...
  def getWeights(self, gameState, action):
    ...
    'normalGhosts': -300, 'eatGhost': 1.0, 'scaredGhosts': 0.1, 'stuck': -10, 'eatFood': 1, 'pathOnExitRoute': 10, 'closeToExitPos': -15, 'repeatMovement': -1}
...
class DefensiveAgent(CaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if(len(invaders) == 0 and successor.getScore() != 0):
      ...
      features['stayApart'] *= features['stayApart']
        ...
        # If we've eaten enough food, try and go to an exit route
    if self.foodNum >= 5 and myState.isPacman: #and (newx, newy) in self.pathToExit:
      # closestExit = self.pathToExit[-1]
      x, y = gameState.getAgentState(self.index).getPosition()
      vx, vy = Actions.directionToVector(action)
      newx = int(x + vx)
  ...
  def getWeights(self,gameState, action):
    ...
    'eatInvader': 5,'teammateDist': 1.5, 'nearbyFood': -5, 'eatCapsule': 10, 'eatGhost': 1.0, 'scaredGhosts': 0.1, 'stuck': -10, 'eatFood': 1, 'pathOnExitRoute': 10, 'repeatMovement': -1}
