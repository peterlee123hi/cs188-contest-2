...
class SmartAgent(CaptureAgent):
    ...
    def getQValue(self, state, action):
        ...
        for f in self.features:
            #print f, self.features[f](self, state, action) * self.weights[f]
    ...
    def computeActionFromQValues(self, state): 
        ...
        #print 'Best action(s):', lst, '\n'
        return random.choice(lst)

    def onSameSide(self, pos):
        #print (self.boundary == 15 and pos[0] <= 15) or (self.boundary == 16 and pos[0] >= 16)
...
class LearningAgent(CaptureAgent):
    ...
    def computeActionFromQValues(self, state): 
        ...
        lst = []
        for a in state.getLegalActions(self.index):
            #print 'trying', self.getQValue(state,a)
    ...
    def update(self, state, action, nextState, reward):
        ...
        print 'maxQ in update is', maxQ
        diff = (reward + self.discount * maxQ) - self.getQValue(state, action)
        print reward, diff
        for f in self.features:
            print self.features[f](self, state, action)
