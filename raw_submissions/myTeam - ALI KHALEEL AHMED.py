...
The offensive bot is also a slightly improved version of the baselineTeam.py model. I add other basic features for the bot such as when to eat the capsule and when to avoid eating the pacman (if you're scared).
"""
...
class OffensiveAgent(CaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    # print "\n"
    # print action
    # print "new position to consider", (newx, newy)
...
class DefensiveAgent(CaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if(len(invaders) == 0 and successor.getScore() != 0):
      features['onDefense'] = -1
      if len(foodList) != 0:
        features['offenseFood'] = min([self.getMazeDistance(myPos,food) for food in foodList])
